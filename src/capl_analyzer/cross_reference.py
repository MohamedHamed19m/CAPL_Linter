...
class CAPLCrossReferenceBuilder:
    ...
    def analyze_file_references(self, file_path: str) -> int:
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            # 1. Function calls
            refs_count += self._extract_function_calls(
                root, source_text, file_id, conn
            )
            
            # 2. Variable usages
            refs_count += self._extract_variable_references(
                root, source_text, file_id, conn
            )
            
            # 3. CAPL-specific: output() calls with messages
            refs_count += self._extract_message_output(
                root, source_text, file_id, conn
            )
            
            # 4. CAPL-specific: setTimer() calls
            refs_count += self._extract_timer_usage(
                root, source_text, file_id, conn
            )
    ...
    def _extract_function_calls(self, root: Node, source: str, 
                                file_id: int, conn) -> int:
    ...
    def _find_containing_function(self, node: Node, source: str, 
                                  file_id: int, conn) -> Optional[int]:
    ...
    def _extract_variable_references(self, root: Node, source: str,
                                    file_id: int, conn) -> int:
    ...
    def _is_declaration(self, node: Node) -> bool:
        ...
        parent = node.parent
        while parent:
            if parent.type in ('function_definition', 'declaration', 
                             'parameter_declaration', 'init_declarator'):
                return True
            parent = parent.parent
        return False
    
    def _extract_message_output(self, root: Node, source: str,
                               file_id: int, conn) -> int:
    ...
    def _extract_timer_usage(self, root: Node, source: str,
                            file_id: int, conn) -> int:
    ...
    def get_call_graph(self, function_name: str) -> Dict[str, List[str]]:
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            callers = [(name, Path(fp).name, line) 
                      for name, fp, line in cursor.fetchall()]
            ...
            callees = [(name, Path(fp).name, line) 
                      for name, fp, line in cursor.fetchall()]
    ...
    def get_message_handlers(self, message_name: str) -> List[Tuple[str, str, int]]:
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            return [(Path(fp).name, name, line) 
                   for fp, name, line in cursor.fetchall()]
    ...
    def get_message_outputs(self, message_name: str) -> List[Tuple[str, str, int]]:
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            return [(Path(fp).name, func or "(global)", line)
                   for fp, func, line in cursor.fetchall()]
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        ...
        for ref in refs[:10]:  # Limit to 10
            print(f"  {Path(ref.file_path).name}:{ref.line_number} "
