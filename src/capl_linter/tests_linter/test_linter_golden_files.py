...
def test_linter_autofix_golden_file(test_name, tmp_path):
    ...
    # 2. Setup DB and Extract Symbols (Neutral Facts)
    ...
    # 4. Compare
    actual_source = work_file.read_text(encoding="utf-8")
    ...
    if actual_source != expected_source:
        diff = difflib.unified_diff(
            ...
            actual_source.splitlines(keepends=True),
            fromfile="expected",
            tofile="actual",
        )
        diff_text = "".join(diff)
        pytest.fail(f"Auto-fix mismatch for {test_name}:\n{diff_text}")